...
class KGNode {
  ...
  const KGNode({required this.id, required this.label, required this.community,
      required this.degree, required this.contentType, this.source});
  factory KGNode.fromJson(Map j) => KGNode(
    id: j['id'] ?? '', label: j['label'] ?? '',
    community: j['community'] ?? 0, degree: j['degree'] ?? 1,
    contentType: j['content_type'] ?? j['type'] ?? 'text', source: j['source'] ?? j['path']);
  Map<String, dynamic> toJson() => {'id': id, 'label': label, 'community': community,
      'degree': degree, 'content_type': contentType, 'source': source};
}
...
class KGCommunity {
  ...
  const KGCommunity({required this.id, required this.nodeCount, required this.name, required this.summary});
  factory KGCommunity.fromJson(Map j) => KGCommunity(
    id: j['id'] ?? 0, nodeCount: j['nodeCount'] ?? 0,
    name: j['name'] ?? 'Cluster ${j['id']}', summary: j['summary'] ?? '');
  Map<String, dynamic> toJson() => {'id': id, 'nodeCount': nodeCount, 'name': name, 'summary': summary};
}

class KGEdge {
  final String source, target, type;
  final double weight;
  const KGEdge({required this.source, required this.target, required this.type, this.weight = 1});
  factory KGEdge.fromJson(Map j) => KGEdge(
    source: j['source'] ?? '', target: j['target'] ?? '',
    type: j['type'] ?? 'direct', weight: (j['weight'] ?? 1).toDouble());
  Map<String, dynamic> toJson() => {'source': source, 'target': target, 'type': type, 'weight': weight};
}
...
class GraphState {
  ...
  const GraphState({this.nodes = const [], this.edges = const [], this.communities = const [],
      this.loading = false, this.selectedId, this.selectedCommunity, this.search = '',
      this.hiddenCommunities = const [], this.activeSource = 'graph', this.error,
      this.initialized = false});
  GraphState copyWith({List<KGNode>? nodes, List<KGEdge>? edges, List<KGCommunity>? communities,
      bool? loading, String? selectedId, int? selectedCommunity, String? search,
      List<int>? hiddenCommunities, String? activeSource, String? error, bool? initialized}) =>
    GraphState(nodes: nodes ?? this.nodes, edges: edges ?? this.edges,
      communities: communities ?? this.communities, loading: loading ?? this.loading,
      selectedId: selectedId ?? this.selectedId, selectedCommunity: selectedCommunity ?? this.selectedCommunity,
      search: search ?? this.search, hiddenCommunities: hiddenCommunities ?? this.hiddenCommunities,
      activeSource: activeSource ?? this.activeSource, error: error,
...
class GraphNotifier extends StateNotifier<GraphState> {
  ...
  Future<void> load({String source = 'graph'}) async {
    state = state.copyWith(loading: true, activeSource: source, error: null);
    try {
      List<KGNode> nodes = [];
      List<KGEdge> edges = [];
      ...
      if (source == 'vault') {
        ...
        nodes = (r.data['nodes'] as List).map((n) => KGNode(
          id: n['id'] ?? '', label: n['title'] ?? n['label'] ?? '',
          community: 0, degree: (n['links'] as List?)?.length ?? 0,
          contentType: 'note', source: n['path'])).toList();
        edges = (r.data['edges'] as List).map((e) => KGEdge.fromJson(e as Map)).toList();
      } else {
        final r = await _dio.get(ApiConstants.graphFull);
        nodes = (r.data['nodes'] as List).map((n) => KGNode.fromJson(n as Map)).toList();
        edges = (r.data['edges'] as List).map((e) => KGEdge.fromJson(e as Map)).toList();
        communities = (r.data['communities'] as List).map((c) => KGCommunity.fromJson(c as Map)).toList();
      ...
      state = state.copyWith(nodes: nodes, edges: edges, communities: communities, loading: false, initialized: true);
    } catch (e) {
      final box = Hive.box('cyborg_cache');
      ...
      if (cachedNodes != null && cachedEdges != null) {
        final nodes = (cachedNodes as List).map((n) => KGNode.fromJson(n as Map)).toList();
        final edges = (cachedEdges as List).map((e) => KGEdge.fromJson(e as Map)).toList();
        final comms = cachedComms != null ? (cachedComms as List).map((c) => KGCommunity.fromJson(c as Map)).toList() : <KGCommunity>[];
        state = state.copyWith(nodes: nodes, edges: edges, communities: comms, loading: false, initialized: true, error: 'Offline mode: Showing cached local graph.');
      } else {
        state = state.copyWith(
            loading: false, initialized: true,
            error: 'Backend offline and no local cache — start the Cyborg backend to load the graph.');
      }
    }
  }
  ...
  void select(String? id) => state = state.copyWith(selectedId: id, selectedCommunity: null);
  void selectCommunity(int? id) => state = state.copyWith(selectedCommunity: id, selectedId: null);
  ...
  Future<void> clearGraph() async {
    try {
      ...
      state = state.copyWith(nodes: [], edges: [], selectedId: null, initialized: true);
    } catch (_) {}
  }
}
...
class KnowledgeGraphScreen extends ConsumerStatefulWidget {
  ...
  @override ConsumerState<KnowledgeGraphScreen> createState() => _KnowledgeGraphScreenState();
}
...
class _KnowledgeGraphScreenState extends ConsumerState<KnowledgeGraphScreen> {
  ...
  @override
  void dispose() { _searchCtrl.dispose(); _ingestCtrl.dispose(); super.dispose(); }
          ...
          child: narrow
              ? Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                  Row(children: [
                    ...
                    const Expanded(child: Text('Knowledge Graph',
                    ...
                    const SizedBox(width: 6),
                    IconButton(icon: const Icon(Icons.upload_file_outlined, size: 18),
                        padding: EdgeInsets.zero,
                        tooltip: 'Ingest files',
                        onPressed: () => _showIngest(context, n)),
                    IconButton(icon: const Icon(Icons.delete_sweep_outlined, size: 18, color: AppColors.accentRed),
                        padding: EdgeInsets.zero,
                        tooltip: 'Clear Graph',
                  ...
                  Row(children: [
                    _SourceToggle(active: s.activeSource,
                        onSwitch: (src) => n.load(source: src)),
                    const SizedBox(width: 8),
                    Expanded(child: SizedBox(height: 30, child: TextField(
                        controller: _searchCtrl, onChanged: n.setSearch,
              ...
              : Row(children: [
                  const Icon(Icons.hub_outlined, color: AppColors.accent, size: 18),
                  const SizedBox(width: 8),
                  const Text('Knowledge Graph', style: TextStyle(
                  ...
                  Container(
                    ...
                    child: Text('${s.nodes.length} nodes · ${s.edges.length} edges',
                  ...
                  SizedBox(width: 200, child: TextField(
                    ...
                    style: const TextStyle(color: AppColors.textPrimary, fontSize: 12),
                    decoration: const InputDecoration(
                        hintText: 'Search nodes...', isDense: true,
                        prefixIcon: Icon(Icons.search, size: 14),
                        contentPadding: EdgeInsets.symmetric(
      ...
      Expanded(child: Row(children: [
        Expanded(child: !s.initialized
            ...
            : s.loading
                ? const Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                    CircularProgressIndicator(), SizedBox(height: 16),
                    Text('Loading graph…', style: TextStyle(color: AppColors.textSecondary))]))
                : s.error != null
                    ? _ErrorGraph(message: s.error!, onRetry: () => n.load(source: s.activeSource))
                    : s.nodes.isEmpty
                        ? _EmptyGraph(onIngest: () => _showIngest(context, n))
                        : _GraphCanvas(nodes: s.nodes, edges: s.edges, search: s.search,
                            hiddenCommunities: s.hiddenCommunities, onNodeTap: n.select,
        ...
        Container(width: 220,
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Padding(padding: EdgeInsets.fromLTRB(12, 12, 12, 4),
              child: Text('COMMUNITIES', style: TextStyle(color: AppColors.textMuted,
                  fontSize: 10, fontWeight: FontWeight.w700, letterSpacing: 1))),
            Expanded(child: ListView(padding: EdgeInsets.zero, children: _communityItems(s, n))),
            const Divider(height: 1),
            if (selected != null)
              SizedBox(height: 180, child: SingleChildScrollView(child: _NodeInfo(node: selected)))
            else if (s.selectedCommunity != null)
              SizedBox(height: 180, child: SingleChildScrollView(child: _CommunityInfo(
                community: s.communities.firstWhere((c) => c.id == s.selectedCommunity,
                    orElse: () => KGCommunity(id: s.selectedCommunity!, nodeCount: 0, name: 'Cluster ${s.selectedCommunity}', summary: 'Loading...')))))
            else
              const Padding(padding: EdgeInsets.all(12), child: Text('Click a node or cluster to inspect',
  ...
  List<Widget> _communityItems(GraphState s, GraphNotifier n) {
    final List<KGCommunity> list = s.communities.isNotEmpty ? s.communities : _fallbackCommunities(s);
    return list.map((c) {
      final hidden = s.hiddenCommunities.contains(c.id);
      final color = AppColors.communityColors[c.id % AppColors.communityColors.length];
      ...
      return Material(
        ...
        child: ListTile(dense: true,
          leading: Container(width: 10, height: 10, decoration: BoxDecoration(
              shape: BoxShape.circle, color: hidden ? color.withOpacity(0.2) : color)),
          title: Text(c.name, style: TextStyle(fontSize: 11, fontWeight: isSelected ? FontWeight.w600 : FontWeight.w400,
              color: hidden ? AppColors.textMuted : AppColors.textSecondary)),
          trailing: Text('${c.nodeCount}', style: const TextStyle(fontSize: 10, color: AppColors.textMuted)),
  ...
  void _showClearConfirm(BuildContext ctx, GraphNotifier n) {
    showDialog(
      ...
      builder: (dialogCtx) => AlertDialog(
        ...
        title: const Text('Clear Knowledge Graph?', style: TextStyle(color: AppColors.textPrimary)),
        content: const Text('This will delete all nodes and edges permanently.', style: TextStyle(color: AppColors.textSecondary)),
        actions: [
    ...
    showDialog(
      ...
      builder: (dialogCtx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            SizedBox(width: 360, child: TextField(controller: _ingestCtrl,
                style: const TextStyle(color: AppColors.textPrimary, fontSize: 13),
                decoration: const InputDecoration(
                    hintText: '/path/to/folder or Device IP/ID',
                    prefixIcon: Icon(Icons.folder_outlined, size: 16)))),
            const SizedBox(height: 12),
            const Text('Supports local folders, files, or connected devices via ADB/mDNS.',
                style: TextStyle(color: AppColors.textMuted, fontSize: 11)),
          ],
        ),
        actions: [
...
class _SourceToggle extends StatelessWidget {
  final String active; final void Function(String) onSwitch;
...
class _Tab extends StatelessWidget {
  final String label, src, active; final void Function(String) fn;
  const _Tab(this.label, this.src, this.active, this.fn);
  @override
  Widget build(BuildContext context) {
    final sel = active == src;
    return GestureDetector(onTap: () => fn(src), child: AnimatedContainer(
      duration: const Duration(milliseconds: 130),
      padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
      decoration: BoxDecoration(
          color: sel ? AppColors.accent.withOpacity(0.15) : Colors.transparent,
...
class _NodeLayout {
  final KGNode node;
  double x, y, vx, vy;
  _NodeLayout(this.node, this.x, this.y) : vx = 0, vy = 0;
}
...
class _GraphCanvas extends StatefulWidget {
  ...
  final String? selectedId;
  const _GraphCanvas({required this.nodes, required this.edges, required this.search,
      required this.hiddenCommunities, required this.onNodeTap, this.selectedId});
  @override State<_GraphCanvas> createState() => _GraphCanvasState();
}

class _GraphCanvasState extends State<_GraphCanvas> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _buildLayouts();
    _ticker = AnimationController(vsync: this, duration: const Duration(seconds: 600))
      ..addListener(_tick)
      ..repeat();
  }
  ...
  void _tick() {
    ...
    final Map<String, _NodeLayout> byId = {for (var l in _layouts) l.node.id: l};
    ...
    for (var i = 0; i < _layouts.length; i++) {
      ...
      final repulsionK = (k * k) / math.sqrt(widget.nodes.length).clamp(1.0, 50.0);
    ...
    for (final edge in widget.edges) {
      ...
      if (a != _draggedNode) { a.vx += dx / dist * force; a.vy += dy / dist * force; }
      if (b != _draggedNode) { b.vx -= dx / dist * force; b.vy -= dy / dist * force; }
    }
...
class _ObsidianPainter extends CustomPainter {
  ...
  const _ObsidianPainter({required this.layouts, required this.edges,
      required this.hiddenCommunities, required this.searchLower,
      this.selectedId, this.hoveredId, required this.scale, required this.pan});

  Offset _p(double x, double y) => Offset(x * scale + pan.dx, y * scale + pan.dy);

  double _r(KGNode n) => (math.sqrt(n.degree.toDouble()) * 2 + 4).clamp(5, 22) * scale;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final edge in edges) {
      ...
      final opacity = searchLower.isNotEmpty ? 0.08 : isHighlighted ? 0.8 : 0.2;
      ...
      final paint = Paint()
        ..color = (edge.type == 'wikilink' ? AppColors.accentPurple : AppColors.accent).withOpacity(opacity)
      ...
      if ((scale > 1.2 || isHighlighted) && edge.type != 'direct' && edge.type != 'related') {
        final mid = (posA + posB) / 2;
        final tp = TextPainter(
          text: TextSpan(text: edge.type, style: TextStyle(
    ...
    for (final l in layouts) {
      ...
      if (isSelected) {
        canvas.drawCircle(pos, r + 6 * scale, Paint()
      ...
      canvas.drawCircle(pos, r, Paint()
        ...
        ..style = PaintingStyle.fill);
      canvas.drawCircle(pos, r, Paint()
        ..color = (isSelected || isHovered ? color : color.withOpacity(0.4)).withOpacity(opacity)
        ..strokeWidth = isSelected ? 2.0 * scale : 1.0 * scale
        ..style = PaintingStyle.stroke);

      final showLabel = isSelected || isHovered || (scale > 0.7 && matchesSearch);
      if (showLabel && l.node.label.isNotEmpty) {
        ...
        final tp = TextPainter(
          text: TextSpan(text: label, style: TextStyle(
              color: isSelected ? color : Colors.white.withOpacity(opacity * 0.85),
...
class _NodeInfo extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      const Text('SELECTED NODE', style: TextStyle(color: AppColors.textMuted, fontSize: 10,
          fontWeight: FontWeight.w700, letterSpacing: 1)),
      const SizedBox(height: 8),
      Text(node.label, style: const TextStyle(color: AppColors.textPrimary, fontSize: 13, fontWeight: FontWeight.w600)),
      ...
      if (node.source != null && node.source!.isNotEmpty)
        _Row('Source', node.source!.length > 30 ? '...${node.source!.substring(node.source!.length - 28)}' : node.source!),
    ]));
}
...
class _CommunityInfo extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
    padding: const EdgeInsets.all(12),
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      const Text('CLUSTER INFO', style: TextStyle(color: AppColors.textMuted, fontSize: 10,
          fontWeight: FontWeight.w700, letterSpacing: 1)),
      const SizedBox(height: 8),
      Text(community.name, style: const TextStyle(color: AppColors.accent, fontSize: 13, fontWeight: FontWeight.w700)),
      const SizedBox(height: 10),
      Text(community.summary.isEmpty ? 'Analyzing cluster content...' : community.summary,
...
class _EmptyGraph extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Center(child: Column(
    ...
    Container(padding: const EdgeInsets.all(24),
      ...
      child: const Icon(Icons.hub_outlined, size: 48, color: AppColors.accent)),
    const SizedBox(height: 20),
    const Text('Empty Knowledge Graph', style: TextStyle(color: AppColors.textPrimary, fontSize: 18, fontWeight: FontWeight.w700)),
    const SizedBox(height: 8),
    const Text('Ingest documents to populate your knowledge graph', style: TextStyle(color: AppColors.textSecondary, fontSize: 13)),
    const SizedBox(height: 24),
    ElevatedButton.icon(icon: const Icon(Icons.upload_file, size: 16),
        label: const Text('Ingest Files'), onPressed: onIngest),
  ]));
}
...
class _ErrorGraph extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Center(child: Column(
    mainAxisAlignment: MainAxisAlignment.center, children: [
    Container(padding: const EdgeInsets.all(24),
      decoration: BoxDecoration(color: AppColors.accentRed.withOpacity(0.08), borderRadius: BorderRadius.circular(20)),
      child: const Icon(Icons.cloud_off_outlined, size: 48, color: AppColors.accentRed)),
    const SizedBox(height: 20),
    const Text('Graph Unavailable', style: TextStyle(color: AppColors.textPrimary, fontSize: 18, fontWeight: FontWeight.w700)),
    const SizedBox(height: 8),
    Padding(
    ...
    const SizedBox(height: 24),
    ElevatedButton.icon(icon: const Icon(Icons.refresh, size: 16),
        label: const Text('Retry'), onPressed: onRetry,
        style: ElevatedButton.styleFrom(backgroundColor: AppColors.accentRed)),
  ]));
}
