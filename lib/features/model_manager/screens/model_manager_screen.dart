...
class ModelInfo {
  ...
  ModelInfo copyWith({
    ...
    id: id, name: name, sizeGb: sizeGb, quantization: quantization,
    taskType: taskType, rating: rating, downloads: downloads,
...
class ModelManagerState {
  ...
  ModelManagerState copyWith({
    ...
    selectedModelForHosting: selectedModelForHosting ?? this.selectedModelForHosting,
...
class ModelManagerNotifier extends StateNotifier<ModelManagerState> {
  ...
  static const _catalog = [
    {'id': 'gemma-4-e2b-it-q8-0', 'name': 'Gemma 4 E2B (Recommended)', 'size_gb': 4.6, 'quantization': 'Q8_0', 'task_type': 'text-generation', 'rating': 5.0, 'downloads': 1000, 'hf_repo': 'google/gemma-2-2b-it-GGUF', 'filename': 'gemma-4-e2b-it-Q8_0.gguf'},
    {'id': 'qwen2.5-1.5b-instruct-q4', 'name': 'Qwen2.5 1.5B Instruct', 'size_gb': 1.1, 'quantization': 'Q4_K_M', 'task_type': 'instruct', 'rating': 4.8, 'downloads': 56000, 'hf_repo': 'Qwen/Qwen2.5-1.5B-Instruct-GGUF', 'filename': 'qwen2.5-1.5b-instruct-q4_k_m.gguf'},
    {'id': 'qwen2.5-coder-14b-q4', 'name': 'Qwen2.5-Coder 14B', 'size_gb': 8.4, 'quantization': 'Q4_K_M', 'task_type': 'code', 'rating': 4.9, 'downloads': 124000, 'hf_repo': 'Qwen/Qwen2.5-Coder-14B-Instruct-GGUF', 'filename': 'qwen2.5-coder-14b-instruct-q4_k_m.gguf'},
    {'id': 'llama-3.2-8b-q4', 'name': 'Llama 3.2 8B', 'size_gb': 4.7, 'quantization': 'Q4_K_M', 'task_type': 'text-generation', 'rating': 4.7, 'downloads': 98000, 'hf_repo': 'meta-llama/Llama-3.2-8B-Instruct-GGUF', 'filename': 'llama-3.2-8b-instruct-q4_k_m.gguf'},
    {'id': 'mistral-7b-q4', 'name': 'Mistral 7B Instruct', 'size_gb': 4.1, 'quantization': 'Q4_K_M', 'task_type': 'instruct', 'rating': 4.6, 'downloads': 89000, 'hf_repo': 'TheBloke/Mistral-7B-Instruct-v0.2-GGUF', 'filename': 'mistral-7b-instruct-v0.2.Q4_K_M.gguf'},
    {'id': 'phi-3.5-mini-q4', 'name': 'Phi-3.5 Mini', 'size_gb': 2.2, 'quantization': 'Q4_K_M', 'task_type': 'text-generation', 'rating': 4.5, 'downloads': 45000, 'hf_repo': 'microsoft/Phi-3.5-mini-instruct-gguf', 'filename': 'Phi-3.5-mini-instruct-Q4_K_M.gguf'},
    {'id': 'deepseek-r1-7b-q4', 'name': 'DeepSeek-R1 7B', 'size_gb': 4.7, 'quantization': 'Q4_K_M', 'task_type': 'reasoning', 'rating': 4.8, 'downloads': 67000, 'hf_repo': 'deepseek-ai/DeepSeek-R1-Distill-Qwen-7B-GGUF', 'filename': 'deepseek-r1-distill-qwen-7b-q4_k_m.gguf'},
    {'id': 'gemma2-9b-q4', 'name': 'Gemma 2 9B', 'size_gb': 5.4, 'quantization': 'Q4_K_M', 'task_type': 'text-generation', 'rating': 4.6, 'downloads': 52000, 'hf_repo': 'google/gemma-2-9b-it-GGUF', 'filename': 'gemma-2-9b-it-Q4_K_M.gguf'},
    {'id': 'codestral-22b-q4', 'name': 'Codestral 22B', 'size_gb': 12.4, 'quantization': 'Q4_K_M', 'task_type': 'code', 'rating': 4.8, 'downloads': 38000, 'hf_repo': 'bartowski/Codestral-22B-v0.1-GGUF', 'filename': 'Codestral-22B-v0.1-Q4_K_M.gguf'},
    {'id': 'moondream2', 'name': 'Moondream2 (Vision)', 'size_gb': 1.8, 'quantization': 'Q8', 'task_type': 'vision', 'rating': 4.4, 'downloads': 29000, 'hf_repo': 'vikhyatk/moondream2', 'filename': 'moondream2-text-model-f16.gguf'},
    {'id': 'gemma4-2b-litert', 'name': 'Gemma 4 2B', 'size_gb': 1.5, 'quantization': 'LiteRT', 'task_type': 'text-generation', 'rating': 4.9, 'downloads': 15000, 'hf_repo': 'litert-community/gemma-4-E2B-it-litert-lm', 'filename': 'gemma-4-E2B-it.litertlm'},
  ];

  void _saveLocalModels(List<ModelInfo> models) {
    _box.put('downloaded_models', jsonEncode(models.map((m) => m.toJson()).toList()));
  }
  ...
  Future<void> _loadModels() async {
    ...
    try {
      ...
      if (data != null) {
        ...
        localModels = list.map((m) => ModelInfo.fromJson(Map<String,dynamic>.from(m))).toList();
    ...
    try {
      ...
      final serverDownloaded = (resp.data['models'] as List)
          .map((m) => ModelInfo.fromJson(m)).toList();

      // Keep any models that have quantization == 'Custom' and are not in serverDownloaded
      final customModels = localModels.where((m) => m.quantization == 'Custom').toList();
  ...
  Future<void> downloadModel(String id) async {
    ...
    try {
      if (Platform.isAndroid || Platform.isIOS) {
        ...
        final catalogEntry = _catalog.firstWhere((e) => e['id'] == id, orElse: () => {});
        ...
        if (id.contains('/')) {
          await _dio.post('${ApiConstants.modelsDownload}/repo', queryParameters: {'repo_id': id});
        } else {
  ...
  Future<void> loadModel(String id) async {
    ...
    final loadingModels = state.downloadedModels.map((m) =>
      m.copyWith(isLoading: m.id == id, isLoaded: m.id == id ? false : m.isLoaded)
    ).toList();
    ...
    try {
      if (Platform.isAndroid || Platform.isIOS) {
        ...
        final loadId = (model.source == 'local' && model.path != null) ? model.path! : model.id;
        final resp = await _dio.post('${ApiConstants.modelsLoad}/$loadId', data: {
          'n_ctx': state.nCtx,
        ...
        if (resp.data['status'] == 'downloading') {
          ref.read(globalErrorProvider.notifier).state = resp.data['message'] ?? 'Model is downloading...';
          return;
        }
      }
      ...
      // Clear loading state
      final reset = state.downloadedModels.map((m) =>
        m.copyWith(isLoading: false)).toList();
  ...
  Future<void> loadCustomModel(String path) async {
    try {
      final modelId = path.split(RegExp(r'[\\/]')).last.replaceAll(RegExp(r'\.(gguf|litertlm|tflite|task|bin)$', caseSensitive: false), '');
      ...
      final downloaded = state.downloadedModels.map((m) =>
        m.copyWith(isLoaded: false)).toList();
...
class _ModelCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final taskColor =
        _taskColors[model.taskType] ?? AppColors.accent;
...
class _DownloadedTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final notifier = ref.read(modelManagerProvider.notifier);
    final loadedModel =
        state.downloadedModels.firstWhere((m) => m.isLoaded,
            orElse: () => const ModelInfo(
                id: '', name: '', sizeGb: 0,
                quantization: '', taskType: ''));
              ...
              ? Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      const Text('Browse the catalog to download models or load manually',
                          style: TextStyle(
                              color: AppColors.textMuted, fontSize: 12)),
                      ...
                      ElevatedButton.icon(
                        ...
                        onPressed: () async {
                          ...
                          if (result != null && result.files.single.path != null) {
                            ...
                            if (ext.endsWith('.gguf') || ext.endsWith('.litertlm') || ext.endsWith('.tflite') || ext.endsWith('.task') || ext.endsWith('.bin')) {
                              try {
                                await ref.read(modelManagerProvider.notifier).loadCustomModel(path);
                              } catch (e) {
                                if (context.mounted) {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(content: Text('Failed to load: $e'), backgroundColor: AppColors.accentRed),
                                  );
                                }
                              }
                            } else {
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(content: Text('Please select a valid model file (.gguf, .litertlm, etc)')),
                              );
                            }
                          }
                        },
                      ...
                      child: Row(
                        ...
                        children: [
                          OutlinedButton.icon(
                            ...
                            label: const Text('Load Model from folder', style: TextStyle(fontSize: 12)),
                            onPressed: () async {
                              final result = await FilePicker.platform.pickFiles(
                                type: FileType.any,
                              );
                  ...
                  itemBuilder: (context, i) {
                    ...
                    return ListTile(
                      ...
                      trailing: Row(
                        ...
                        children: [
                          IconButton(
                            icon: const Icon(Icons.folder_open_outlined, size: 18, color: AppColors.textSecondary),
                            ...
                            onPressed: () {
                              if (model.path != null) {
                                if (Platform.isWindows) {
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(content: Text('File exploring is only supported on Windows.')),
                                  );
                                }
                              } else {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(content: Text('Model path is unknown.')),
                                );
                              }
                            },
                          ),
...
class _ServerTabState extends ConsumerState<_ServerTab> {
  ...
  void initState() {
    ...
    _urlController = TextEditingController(
      text: state.serverUrl.contains(':') ? 'http://${state.serverUrl}/v1' : 'http://localhost:1234/v1'
    );
  }
                      ...
                      ElevatedButton(
                        onPressed: isStarting ? null : () {
                          if (_urlController.text.isNotEmpty) {
                            notifier.updateExternalServer(_urlController.text);
                          }
                        },
                        style: ElevatedButton.styleFrom(
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                          ...
                          child: DropdownButtonHideUnderline(
                            child: DropdownButton<String>(
                              value: state.selectedModelForHosting ??
                                     (state.downloadedModels.isNotEmpty ? state.downloadedModels.first.path : null),
                              isExpanded: true,
                              dropdownColor: AppColors.surface,
                              style: const TextStyle(color: AppColors.textPrimary, fontSize: 13),
                              hint: const Text('Select a model...', style: TextStyle(color: AppColors.textMuted)),
                              onChanged: isRunning || isStarting ? null : (val) {
                  ...
                  _ConfigInputRow('Context Length', state.nCtx.toString(), (val) => notifier.setNCtx(int.tryParse(val) ?? 4096)),
                  _ConfigInputRow('GPU Layers (-1=Auto)', state.nGpuLayers.toString(), (val) => notifier.setNGpuLayers(int.tryParse(val) ?? -1)),
                  _ConfigInputRow('Threads (0=Auto)', state.nThreads.toString(), (val) => notifier.setNThreads(int.tryParse(val) ?? 0)),
                  _ConfigInputRow('Batch Size', state.nBatch.toString(), (val) => notifier.setNBatch(int.tryParse(val) ?? 512)),
                ],
              ),
            ),
          ),
