...
class DeviceManagerScreen extends ConsumerStatefulWidget {
  ...
  @override ConsumerState<DeviceManagerScreen> createState() => _DeviceManagerScreenState();
}
class _DeviceManagerScreenState extends ConsumerState<DeviceManagerScreen> {
  ...
  Future<void> _scan() async {
    ...
    try {
      ...
      setState(() {
        _devices = [{
          'name': 'This Device', 'type': 'local', 'status': 'online',
  ...
  Widget build(BuildContext context) {
    return Column(children: [
      Container(height: 52, padding: const EdgeInsets.symmetric(horizontal: 16),
        ...
        child: Row(children: [
          const Icon(Icons.devices_outlined, color: AppColors.accentOrange, size: 18),
          const SizedBox(width: 8),
          const Text('Device Manager', style: TextStyle(color: AppColors.textPrimary,
              fontSize: 15, fontWeight: FontWeight.w600)),
          const Spacer(),
          ElevatedButton.icon(
            icon: _scanning ? const SizedBox(width: 14, height: 14,
                child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
                : const Icon(Icons.wifi_find, size: 14),
            label: Text(_scanning ? 'Scanning...' : 'Scan Network', style: const TextStyle(fontSize: 12)),
      ...
      Expanded(child: LayoutBuilder(builder: (ctx, constraints) {
        ...
        final content = _devices.isEmpty
            ? const Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                Icon(Icons.devices_outlined, size: 48, color: AppColors.textMuted),
                SizedBox(height: 12),
                Text('No devices found. Click Scan to search.', style: TextStyle(color: AppColors.textSecondary))]))
            : GridView.builder(padding: const EdgeInsets.all(16),
                gridDelegate: SliverGridDelegateWithMaxCrossAxisExtent(
                    maxCrossAxisExtent: narrow ? 400 : 280, mainAxisSpacing: 12, crossAxisSpacing: 12, childAspectRatio: 1.5),
        ...
        final sidebar = Container(width: narrow ? double.infinity : 240,
            decoration: BoxDecoration(color: AppColors.surface,
                border: Border(
                  left: narrow ? BorderSide.none : const BorderSide(color: AppColors.border),
                  top: narrow ? const BorderSide(color: AppColors.border) : BorderSide.none,
                )),
            padding: const EdgeInsets.all(16),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
              const Text('FEATURES', style: TextStyle(color: AppColors.textMuted, fontSize: 10,
              ...
              ].map((f) => Padding(padding: const EdgeInsets.only(bottom: 10), child: Row(children: [
                Icon(f.$1, size: 15, color: AppColors.accentOrange),
                const SizedBox(width: 10),
                Text(f.$2, style: const TextStyle(color: AppColors.textSecondary, fontSize: 12)),
              ]))),
              const Divider(),
              Container(padding: const EdgeInsets.all(10),
                decoration: BoxDecoration(color: AppColors.accentYellow.withOpacity(0.08),
                    borderRadius: BorderRadius.circular(8),
                    border: Border.all(color: AppColors.accentYellow.withOpacity(0.2))),
                child: const Text('Install Cyborg on other devices and they will appear here automatically via mDNS.',
                    style: TextStyle(color: AppColors.accentYellow, fontSize: 11, height: 1.5))),
            ]));
...
class _DeviceCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Card(child: Padding(padding: const EdgeInsets.all(14), child: Column(
      ...
      Row(children: [
        Icon(isLocal ? Icons.computer_outlined : Icons.smartphone_outlined,
            size: 20, color: AppColors.accentOrange),
        const SizedBox(width: 8),
        Expanded(child: Text(device['name'] as String? ?? 'Unknown',
            overflow: TextOverflow.ellipsis,
            style: const TextStyle(color: AppColors.textPrimary, fontSize: 13, fontWeight: FontWeight.w600))),
        Container(width: 8, height: 8, decoration: BoxDecoration(
            shape: BoxShape.circle, color: isOnline ? AppColors.accentGreen : AppColors.accentRed)),
      ]),
      const SizedBox(height: 8),
      Text(device['ip'] as String? ?? '', style: const TextStyle(color: AppColors.textMuted, fontSize: 11)),
      const SizedBox(height: 12),
      Row(children: [
        ...
        TextButton.icon(
          ...
          onPressed: () {
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
              content: Text('Device added to Knowledge Graph ingestion queue.'),
...
class _MicroBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Text('$label ${value.toStringAsFixed(0)}%', style: TextStyle(color: color, fontSize: 9, fontWeight: FontWeight.w600)),
    const SizedBox(height: 2),
    ClipRRect(borderRadius: BorderRadius.circular(2), child: LinearProgressIndicator(
        value: value / 100, minHeight: 4, backgroundColor: AppColors.surfaceVariant,
