...
class SimulationScreen extends StatefulWidget {
  ...
  @override State<SimulationScreen> createState() => _SimulationScreenState();
}

class _SimulationScreenState extends State<SimulationScreen> {
  final _reqCtrl = TextEditingController(
    text: 'What would be the public opinion trend if Wuhan University issued a notice revoking the disciplinary action against Xiao');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Column(children: [
        MFTopBar(
          currentStep: 3, stepName: 'Simulation',
...
class _PlatformStatusBar extends StatelessWidget {
        ...
        child: Row(children: [
          ...
          if (!p.simRunning && p.plazaRound > 0)
            ElevatedButton(
              ...
              child: const Text('GENERATE REPORT →', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold)),
            ),
        ]),
      ),
    );
  ...
  void _showReportDialog(BuildContext context, AppProvider p) {
    ...
    showDialog(context: context, builder: (dialogContext) => AlertDialog(
      title: const Text('Generate Analysis Report', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
      content: SizedBox(width: 400, child: Column(mainAxisSize: MainAxisSize.min, children: [
        const Text('Enter your research requirement:', style: TextStyle(fontSize: 12, color: MFColors.textSecond)),
        const SizedBox(height: 10),
        TextField(controller: ctrl, maxLines: 3,
          decoration: InputDecoration(
            border: OutlineInputBorder(borderRadius: BorderRadius.circular(4)),
            hintText: 'What would be the public opinion trend if...',
      ...
      actions: [
        ...
        ElevatedButton(
          ...
          style: ElevatedButton.styleFrom(backgroundColor: MFColors.textPrimary, foregroundColor: Colors.white),
...
class _PlatformCard extends StatelessWidget {
  ...
  const _PlatformCard({
    ...
    required this.acts, required this.done,
  });
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        border: Border.all(color: done ? MFColors.accentGreen.withOpacity(0.4) : MFColors.border),
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Text(label, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 0.5)),
            const Spacer(),
            if (done) const Icon(Icons.check, size: 12, color: MFColors.accentGreen),
          ]),
          const SizedBox(height: 2),
          Text('ROUND $round/$totalRounds   ELAPSED TIME ${round}h 0m   ACTS $acts',
  ...
  Widget build(BuildContext context) {
    return Column(children: [
      ...
      Container(
        ...
        decoration: const BoxDecoration(border: Border(bottom: BorderSide(color: MFColors.border))),
        child: Row(children: [
          const Text('TOTAL EVENTS:', style: TextStyle(fontSize: 11, color: MFColors.textSecond)),
          const SizedBox(width: 6),
          Text('${p.totalEvents}', style: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold)),
          ...
          const Icon(Icons.chat_bubble_outline, size: 12, color: MFColors.textSecond),
          const SizedBox(width: 4),
      ...
      Expanded(
        child: p.simEvents.isEmpty
          ? Center(child: p.simRunning
              ? const Column(mainAxisSize: MainAxisSize.min, children: [
                  CircularProgressIndicator(strokeWidth: 2, color: MFColors.accentGreen),
                  SizedBox(height: 12),
                  Text('Simulation running...', style: TextStyle(color: MFColors.textSecond, fontSize: 12)),
                ])
              : const Text('No events yet. Start simulation to see activity.',
          ...
          : ListView.separated(
              ...
              itemBuilder: (_, i) => _EventCard(event: p.simEvents[p.simEvents.length - 1 - i]),
            ),
      ),
    ]);
  }
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Container(
            ...
            child: Center(child: Text(
              event.agentName.isNotEmpty ? event.agentName[0].toUpperCase() : 'A',
              style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold,
                color: MFColors.nodeColor(event.agentType)))),
          ),
          const SizedBox(width: 8),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(event.agentName, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600)),
            Text(event.agentType.toUpperCase(),
              style: const TextStyle(fontSize: 9, color: MFColors.textMuted, letterSpacing: 0.5)),
          ])),
          // Platform icon
          Icon(
            event.platform == 'plaza' ? Icons.language : Icons.chat_bubble_outline,
          ...
          Container(
            ...
            child: Text(event.eventType.toUpperCase(),
              style: const TextStyle(fontSize: 8, color: MFColors.textSecond)),
          ),
        ]),
        ...
        Container(
          ...
          child: Text(event.content, style: const TextStyle(fontSize: 12, height: 1.5, color: MFColors.textPrimary)),
        ),
        const SizedBox(height: 4),
        Text(event.time, style: const TextStyle(fontSize: 10, color: MFColors.textMuted)),
      ]),
    );
  }
}
