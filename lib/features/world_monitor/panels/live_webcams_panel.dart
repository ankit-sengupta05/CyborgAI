...
class LiveWebcamsPanel extends StatefulWidget {
  ...
  @override
  State<LiveWebcamsPanel> createState() => _LiveWebcamsPanelState();
}
...
class _LiveWebcamsPanelState extends State<LiveWebcamsPanel> {
  ...
  Widget build(BuildContext context) => GridPanel(
        ...
        onClose: () =>
            context.read<DashboardProvider>().togglePanel(PanelId.liveWebcams),
        headerActions: [
          Container(
        ...
        child: Column(children: [
          ...
          Expanded(
              child: _expandedIdx != null ? _buildExpanded() : _buildGrid()),
        ]),
      );
        ...
        return GestureDetector(
          ...
          child: Stack(children: [
            ...
            Positioned(
                bottom: 3,
                left: 4,
                child: Text(cam['country'] as String,
                    style: const TextStyle(
                        color: WMColors.textSecond, fontSize: 6))),
            Positioned(
                bottom: 3,
                right: 4,
              ...
              return GestureDetector(
                  ...
                  child: Container(
                      ...
                      decoration: BoxDecoration(
                          border: Border.all(
                              color:
                                  sel ? WMColors.accentGreen : WMColors.border,
                              width: sel ? 2 : 1)),
                      child: Stack(children: [
                        _AnimCam(
...
class _CatBar extends StatelessWidget {
  ...
  const _CatBar(
      {required this.cats, required this.current, required this.onSelect});
  @override
  Widget build(BuildContext ctx) => Container(
      ...
      child: SingleChildScrollView(
          ...
          child: Row(children: [
            GestureDetector(
                onTap: () => onSelect('IRAN ATTACKS'),
                child: Container(
                    padding:
                        const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                    color: current == 'IRAN ATTACKS'
                        ? WMColors.accentRed
                        : WMColors.accentRed.withOpacity(0.25),
                    child: const Text('IRAN ATTACKS',
                        style: TextStyle(
                            color: Colors.white,
                            fontSize: 8,
                            fontWeight: FontWeight.bold)))),
            const SizedBox(width: 4),
            ...cats.skip(1).map((c) => Padding(
                ...
                child: GestureDetector(
                    ...
                    child: Container(
                        ...
                        decoration: BoxDecoration(
                            color: current == c
                                ? WMColors.accentGreen.withOpacity(0.15)
                                : Colors.transparent,
                            border: Border.all(
                                color: current == c
...
class _AnimCam extends StatefulWidget {
  ...
  @override
  State<_AnimCam> createState() => _AnimCamState();
}

class _AnimCamState extends State<_AnimCam>
    with SingleTickerProviderStateMixin {
  late AnimationController _c;
  @override
  void initState() {
    super.initState();
    _c = AnimationController(duration: const Duration(seconds: 4), vsync: this)
      ..repeat();
  }
  ...
  Widget build(BuildContext ctx) => AnimatedBuilder(
      ...
      builder: (_, __) => CustomPaint(
          painter: _CP(color: widget.color, t: _c.value, city: widget.city),
...
class _CP extends CustomPainter {
  ...
  final String city;
  final Random _r;
  _CP({required this.color, required this.t, required this.city})
      : _r = Random(city.hashCode);
  @override
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < cnt; i++) {
      ...
      final x = i * (size.width / cnt);
      final w = 7.0 + seed % 5;
      final h = 15.0 + seed % 35;
      ...
      for (double wy = size.height - h - 4; wy < size.height - 10; wy += 5)
        for (double wx = x + 3; wx < x + w - 1; wx += 4)
          if ((seed + wy.toInt() + wx.toInt()) % 4 != 0)
            canvas.drawRect(Rect.fromLTWH(wx, wy, 2, 2), wp);
    }
    final sp = Paint()
    ...
    canvas.drawRect(
        Rect.fromLTWH(sx - 30, 0, 60, size.height),
        Paint()
  ...
  @override
  bool shouldRepaint(_CP o) => o.t != t;
}
