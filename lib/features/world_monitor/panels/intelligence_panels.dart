...
class AiInsightsPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return GridPanel(
      ...
      isLive: true,
      onClose: () =>
          context.read<DashboardProvider>().togglePanel(PanelId.aiInsights),
      headerActions: [
        Container(
            ...
            child: Row(children: [
              Container(
                  ...
                  decoration: const BoxDecoration(
                      color: WMColors.accentBlue, shape: BoxShape.circle)),
              const SizedBox(width: 8),
              const Text('WORLD BRIEF',
                  style: TextStyle(
  ...
  String _generateBrief(List<NewsArticle> news) {
    ...
    final conflict = news
        .where((n) => n.category == 'CONFLICT' || n.category == 'MILITARY')
        .take(2)
        .toList();
    final markets = news.where((n) => n.category == 'MARKETS').take(1).toList();
    final buf = StringBuffer();
    for (final n in conflict) {
      buf.write('${n.title}. ');
    }
    for (final n in markets) {
      buf.write('On the financial front, ${n.title.toLowerCase()}. ');
    }
    buf.write(
        'Global risk index currently elevated. Monitoring ${news.length} live intelligence feeds across all regions.');
    return buf.toString();
  }
  ...
  List<Widget> _buildIndicators(DataService ds) {
    ...
    return top.take(6).map((c) {
      final color = c.instabilityScore >= 70
          ? WMColors.accentRed
          : c.instabilityScore >= 50
...
class _AiBriefText extends StatefulWidget {
  ...
  @override
  State<_AiBriefText> createState() => _AiBriefTextState();
}
...
class AiForecastsPanel extends StatefulWidget {
  ...
  @override
  State<AiForecastsPanel> createState() => _AiForecastsPanelState();
}
...
class _AiForecastsPanelState extends State<AiForecastsPanel> {
  ...
  List<Map<String, dynamic>> _forecasts(DataService ds) {
    ...
    final base = [
      {
        'title': 'Iran-Israel direct confrontation probability rising',
        'cat': 'Conflict',
        'region': 'MENA',
        'prob': 67,
        'tf': '72h',
      ...
      {
        'title': 'Russian Baltic maritime provocation likely',
        'cat': 'Military',
        'region': 'Europe',
        'prob': 54,
        'tf': '7d',
      ...
      {
        'title': 'EU energy grid cyber-attack risk elevated',
        'cat': 'Cyber',
        'region': 'Europe',
        'prob': 41,
        'tf': '14d',
      ...
      {
        'title': 'DPRK missile test window approaching',
        'cat': 'Military',
        'region': 'East Asia',
        'prob': 38,
        'tf': '30d',
      ...
      {
        'title': 'India-Pakistan border escalation',
        'cat': 'Military',
        'region': 'South Asia',
        'prob': 23,
        'tf': '30d',
      ...
      {
        'title': 'Venezuela regime instability peak',
        'cat': 'Political',
        'region': 'LatAm',
        'prob': 45,
        'tf': '7d',
      ...
      {
        'title': 'Taiwan Strait tension spike',
        'cat': 'Military',
        'region': 'East Asia',
        'prob': 32,
        'tf': '30d',
      ...
      {
        'title': 'Global shipping disruption via Red Sea',
        'cat': 'Supply Chain',
        'region': 'MENA',
        'prob': 58,
        'tf': '7d',
      ...
      {
        'title': 'US-China semiconductor decoupling escalates',
        'cat': 'Market',
        'region': 'East Asia',
        'prob': 71,
        'tf': '30d',
    ...
    for (final f in base) {
      ...
      final related = ds.countries.where((c) {
        ...
        if (region == 'Europe')
          return ['Ukraine', 'Russia', 'Germany', 'Poland'].contains(c.name);
        if (region == 'East Asia')
          return ['China', 'North Korea', 'South Korea', 'Japan']
              .contains(c.name);
        if (region == 'South Asia')
          return ['Pakistan', 'India', 'Afghanistan'].contains(c.name);
        if (region == 'LatAm')
          return ['Venezuela', 'Colombia', 'Brazil'].contains(c.name);
        return false;
      }).toList();
      if (related.isNotEmpty) {
        final avg =
            related.map((c) => c.instabilityScore).reduce((a, b) => a + b) /
                related.length;
        f['prob'] =
            ((f['prob'] as int) + (avg * 0.08 - 4).round()).clamp(5, 95);
      }
    }
    return base;
  }
  ...
  Widget build(BuildContext context) {
    ...
    return GridPanel(
      ...
      onClose: () =>
          context.read<DashboardProvider>().togglePanel(PanelId.aiForecasts),
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            ...
            const Spacer(),
            Text('${data['trend']} ${data['tf']}',
                style: TextStyle(
                    color: _color, fontSize: 8, fontWeight: FontWeight.bold)),
          ]),
        ]),
      );
}
  ...
  Widget build(BuildContext context) {
    ...
    return GridPanel(
      ...
      onClose: () => context
          ...
          .togglePanel(PanelId.countryInstability),
      headerActions: [
        Container(
          ...
          child: const Icon(Icons.help_outline,
              color: WMColors.textMuted, size: 9),
        ),
      ],
      child: ListView.builder(
        padding: const EdgeInsets.all(6),
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          ClipRRect(
              borderRadius: BorderRadius.circular(1),
              child: LinearProgressIndicator(
                value: country.instabilityScore / 100,
                backgroundColor: WMColors.border,
                valueColor: AlwaysStoppedAnimation(_color),
                minHeight: 4,
              )),
          ...
          Text(
            ...
            style: const TextStyle(
                color: WMColors.textMuted, fontSize: 7, letterSpacing: 0.4),
          ),
        ]),
      );
}
...
class StrategicRiskPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final ds = context.watch<DataService>();
    final avgScore = ds.countries.isEmpty
        ? 50.0
        : ds.countries.map((c) => c.instabilityScore).reduce((a, b) => a + b) /
            ds.countries.length;
    ...
    return GridPanel(
      ...
      isLive: true,
      onClose: () =>
          context.read<DashboardProvider>().togglePanel(PanelId.strategicRisk),
          ...
          child: Column(children: [
            ...
            Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              const Icon(Icons.arrow_forward,
                  color: WMColors.accentBlue, size: 14),
              const SizedBox(width: 6),
...
class _Gauge extends StatelessWidget {
  ...
  Widget build(BuildContext context) => CustomPaint(
        ...
        child: Center(
          ...
          Text(_label,
              style:
                  TextStyle(color: _color, fontSize: 10, letterSpacing: 1.5)),
        ])),
      );
}
...
class _GaugePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawArc(Rect.fromCircle(center: Offset(cx, cy), radius: r), sa, sw,
        false, base);
    canvas.drawArc(Rect.fromCircle(center: Offset(cx, cy), radius: r), sa,
        sw * (score / 100), false, glow);
    canvas.drawArc(Rect.fromCircle(center: Offset(cx, cy), radius: r), sa,
        sw * (score / 100), false, fill);
  }
...
class AiStrategicPosturePanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) => GridPanel(
        ...
        onClose: () => context
            .read<DashboardProvider>()
            .togglePanel(PanelId.aiStrategicPosture),
        headerActions: [
          Container(
            padding: const EdgeInsets.all(3),
            decoration: BoxDecoration(
          ...
          Container(
            ...
            child: const Row(children: [
              Icon(Icons.lightbulb_outline,
                  color: WMColors.accentYellow, size: 12),
              SizedBox(width: 6),
              Text('Emoji Key',
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 4),
        Text('→ ${data['trend']}',
            style: const TextStyle(color: WMColors.textMuted, fontSize: 8)),
      ]),
    );
  }
}
