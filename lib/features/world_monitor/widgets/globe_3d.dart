...
class _GlobePainter extends CustomPainter {
  ...
  void _drawContinents(Canvas canvas, double cx, double cy, double r) {
    ...
    _fillContinent(
        ...
        r,
        _conflictMiddleEast,
        WMColors.highAlert.withOpacity(0.22),
        WMColors.highAlert.withOpacity(0.5));
    _fillContinent(
        canvas,
        cx,
        cy,
        r,
        _conflictUkraine,
        WMColors.highAlert.withOpacity(0.28),
        WMColors.highAlert.withOpacity(0.6));
    _fillContinent(
        canvas,
        cx,
        cy,
        r,
        _conflictSudan,
        WMColors.accentOrange.withOpacity(0.22),
        WMColors.accentOrange.withOpacity(0.5));
  }

  void _fillContinent(Canvas canvas, double cx, double cy, double r,
      List<List<double>> coords, Color fill, Color border) {
    final pts = coords
        .map((c) => _project(c[1] * pi / 180, c[0] * pi / 180, cx, cy, r))
        .whereType<Offset>()
        .toList();
  ...
  void _drawArc(Canvas canvas, double cx, double cy, double r,
      List<dynamic> arc, int idx) {
    final lat1 = arc[0] as double;
    final lng1 = arc[1] as double;
    final lat2 = arc[2] as double;
    final lng2 = arc[3] as double;
  ...
  void _drawHotspot(Canvas canvas, double cx, double cy, double r, _Hotspot h) {
    ...
    canvas.drawCircle(
        ...
        sz + 3,
        Paint()
          ..color = h.color.withOpacity(0.15 * rz)
