...
class IngestState {
  ...
  const IngestState(
      {this.jobs = const [], this.isRunning = false, this.totalNodes = 0});
  IngestState copyWith(
          {List<IngestJob>? jobs, bool? isRunning, int? totalNodes}) =>
      IngestState(
          jobs: jobs ?? this.jobs,
...
class IngestNotifier extends StateNotifier<IngestState> {
  ...
  void addYouTube(String url) {
    ...
    _add(
        IngestJob(id: _nextId(), name: name, path: url, type: JobType.youtube));
  }
  ...
  Future<void> _processNext() async {
    ...
    final queued =
        state.jobs.where((j) => j.status == JobStatus.queued).toList();
    if (queued.isEmpty) return;

    final job = queued.first;
    _updateJob(job.id,
        status: JobStatus.running, statusText: 'Processing…', progress: 0.05);
  ...
  Future<void> _ingestPath(IngestJob job) async {
    ...
    final progressTimer =
      ...
      final currentJob = state.jobs.firstWhere((j) => j.id == job.id);
      if (currentJob.progress < 0.9) {
        _updateJob(job.id,
            progress: currentJob.progress + 0.02,
            statusText: 'Extracting knowledge...');
      }
    });
  ...
  Future<void> _ingestYouTube(IngestJob job) async {
    _updateJob(job.id, statusText: 'Extracting transcript…', progress: 0.1);
...
class _IngestScreenState extends ConsumerState<IngestScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final running =
        state.jobs.where((j) => j.status == JobStatus.running).length;
        ...
        child: Column(children: [
          Padding(
            ...
            child: Row(children: [
              ...
              if (running > 0)
                _StatPill('$running running', AppColors.accentGreen),
              ...
              child: Column(
                  ...
                  children: [
                    ...
                    _AddButton(
                      ...
                      label: 'Add Files',
                      subtitle:
                          'Pick individual files (.txt, .pdf, .md, .py …)',
                    ...
                    // Stats
                    _StatRow('Total nodes created', '${state.totalNodes}',
                        AppColors.accent),
                    const SizedBox(height: 6),
                    _StatRow('Jobs completed', '$done', AppColors.accentGreen),
                    _StatRow('Jobs failed', '$failed',
                        failed > 0 ? AppColors.accentRed : AppColors.textMuted),
                  ]),
            ),
          ),
          ...
          Expanded(
            child: state.jobs.isEmpty
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      final job =
                          state.jobs[state.jobs.length - 1 - i]; // newest first
                      return _JobCard(
                          job: job, onRemove: () => notifier.removeJob(job.id));
                    },
                  ),
          ),
        ]),
    ...
    return Card(
        ...
        child: Padding(
            ...
            child:
              ...
              Row(children: [
                Icon(_typeIcons[job.type] ?? Icons.file_present,
...
class _EmptyQueue extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Center(
        ...
        const SizedBox(height: 8),
        const Text(
            'Add files, folders, or YouTube URLs\nfrom the panel on the left.',
