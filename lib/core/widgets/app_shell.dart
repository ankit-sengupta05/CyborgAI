...
class _CyborgSidebarState extends ConsumerState<_CyborgSidebar> {
  ...
  static const _navItems = [
    _NavItem(icon: Icons.chat_bubble_outline,  label: 'Chat',            path: '/chat',     group: 'main'),
    _NavItem(icon: Icons.hub_outlined,         label: 'Knowledge Graph', path: '/graph',    group: 'knowledge'),
    _NavItem(icon: Icons.upload_file_outlined, label: 'Ingest',          path: '/ingest',   group: 'knowledge'),
    _NavItem(icon: Icons.edit_note_outlined,   label: 'Vault',           path: '/vault',    group: 'knowledge'),
    _NavItem(icon: Icons.memory_outlined,      label: 'Models',          path: '/models',   group: 'ai'),
    _NavItem(icon: Icons.task_alt_outlined,    label: 'GSD Projects',    path: '/gsd',      group: 'work'),
    _NavItem(icon: Icons.code_outlined,        label: 'CodeFlow',        path: '/codeflow', group: 'work'),
    _NavItem(icon: Icons.waves_outlined,       label: 'MiroFish',        path: '/mirofish', group: 'work'),
    _NavItem(icon: Icons.public_outlined,      label: 'World Monitor',   path: '/monitor',  group: 'monitor'),
    _NavItem(icon: Icons.devices_outlined,     label: 'Devices',         path: '/devices',  group: 'monitor'),
    _NavItem(icon: Icons.cloud_sync_outlined,  label: 'GitHub',          path: '/github',   group: 'tools'),
  ];
  ...
  Widget _buildFooter(AsyncValue<BackendProgress> status) {
    ...
    final dotColor = running ? AppColors.accentGreen
    ...
    return Column(
      children: [
              ...
              if (_expanded) ...[
                ...
                if (running)
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: cudaActive ? AppColors.accent.withOpacity(0.15) : AppColors.textMuted.withOpacity(0.1),
                      borderRadius: BorderRadius.circular(4),
                      border: Border.all(color: cudaActive ? AppColors.accent.withOpacity(0.3) : AppColors.border, width: 0.5),
                    ),
                    child: Text(
                      cudaActive ? 'GPU' : 'CPU',
                      style: TextStyle(
        ...
        ListTile(
          contentPadding:
              EdgeInsets.symmetric(horizontal: _expanded ? 12 : 8),
...
class _NavItem {
  ...
  const _NavItem({required this.icon, required this.label,
      required this.path, required this.group});
}
...
class _NavTileState extends State<_NavTile> {
  ...
  Widget build(BuildContext context) {
    final bg = widget.isActive
        ...
        : _hovered ? AppColors.surfaceVariant : Colors.transparent;
    final iconColor =
