...
def check_file(filepath):
    ...
    if lang == 'python':
        ...
        if IS_GITHUB:
            return
        # Try a basic syntax check (kotlinc doesn't have a simple one,
        # but we can try to compile to a temp jar)
        run(['kotlinc', p, '-d', 'temp.jar'], 'Kotlin', p)
        if Path('temp.jar').exists():
            os.remove('temp.jar')
